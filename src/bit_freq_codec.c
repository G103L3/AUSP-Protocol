...
struct_out_tones frequency_coder(int bit, int role){
    ...
    if (role == 0) { // Master
        ...
        out_tones.tones[1] = MASTER_BASE + (TONE_STEP * 18); //4600 è la frequenza portante per il master
    } else if (role == 1) { // Slave
        out_tones.tones[0] = SLAVE_BASE + (bit * TONE_STEP); //Frequenza di segnale per lo slave
        out_tones.tones[1] = SLAVE_BASE + (TONE_STEP * 18); //8600 è la frequenza portante per lo slave
    } else if (role== 2) { // Configuration
        out_tones.tones[0] = CONFIG_BASE + (bit * TONE_STEP); //Frequenza di segnale per il config
        out_tones.tones[1] = CONFIG_BASE + (TONE_STEP * 18); //12800 è la frequenza portante per il config
    }
